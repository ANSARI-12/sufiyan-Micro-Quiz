const App = () => {
    ...
    const [user, setUser] = React.useState({ name: '', id: null, score: 0 });
    ...
    const calculateAndShowResults = async () => {
        ...
        try {
            await fetch('http://localhost:3001/api/results', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ userId: user.id, score: score })
            });
        } catch (err) {
    ...
    const renderView = () => {
        switch (currentView) {
            ...
            case 'results':
                const isPassed = user.score >= Math.ceil(quizData.length * 0.7);
                return (
